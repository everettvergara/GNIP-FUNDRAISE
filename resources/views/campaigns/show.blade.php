...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        @include('campaigns.partials.show-content', ['campaign' => $campaign, 'showDonateButton' => true])
                            ...
                            @if ($report->photos->isNotEmpty())
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                                    @foreach ($report->photos as $photo)
                                        <img src="{{ asset('storage/'.$photo->path) }}" alt="" class="w-full h-40 object-cover rounded-lg" loading="lazy" decoding="async">
                                    @endforeach
                                </div>
                            @endif
                        </article>
